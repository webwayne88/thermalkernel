...
def print_building_report(building: Building) -> None:
    ...
    print(f"  Слагаемые (кВт·ч/год):")
    print(f"    Q_потери (теплопотери за отопит. период) = {result.q_losses_kwh:.1f}")
    print(f"    Q_быт   (бытовые теплопоступления)      = {result.q_internal_gains_kwh:.1f}")
    print(f"    Q_нетто (после вычета поступлений/КПД)  = {result.q_net_kwh:.1f}")
